...
def plot_rdif(dft, ptitlet):
    ...
    ax = sns.heatmap(t, vmin=min, vmax=max, cmap="vlag", cbar_kws={'label': heatmap_label}, xticklabels=xfreq+1)
...
ptitle = f'test {stname}'+r' O$_3$'
min=-5
max=5
df1['RDif_UcIntLin'] = 100 * (np.asarray(df1.PO3_UcIntLin_etabkgtpumpphigr) - np.asarray(df1.PO3_UcIntLin)) / np.asarray(df1.PO3_UcIntLin)
...
ptitle = f'Homogenized vs Non-Homogenized {stname}'+r' O$_3$'
min=-5
max=5
...
ptitle = f'{stname} Effect of Conversion Efficiency Correction'
min=-5
max=5
...
ptitle = f'{stname} Effect of Background Current Correction'
min=-5
max=5
...
ptitle = f'{stname} Effect of Pump Temperature Correction'
min=-5
max=5
...
ptitle = f'{stname} Effect of Pump Flow Rate (humidity) Correction'
min=-5
max=5
...
ptitle = f'{stname} Effect of Pump Flow Efficiency Correction'
min=-5
max=5
...
ptitle = f'{stname} Effect all DQA Corrections'
min=-5
max=5
