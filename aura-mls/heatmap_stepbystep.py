...
# dfm = pd.read_csv('/home/poyraden/Analysis/Homogenization_public/Files/madrid/DQA_upd/Madrid_Metada_DQA.csv')
...
path = '/home/poyraden/Analysis/Homogenization_public/Files/madrid/DQA_final/Binned/'

df1 = pd.read_csv(path + 'MadridInterpolated_dqa_rs80_finalfinal.csv')
df2 = pd.read_csv(path + 'MadridInterpolated_dqa_nors80.csv')


print('one', len(df1), len(df2))
...
#
# d1 = df1.resample('1W').first().fillna(0)
# d2 = df2.resample('1W').first().fillna(0)
# d2 = df2.resample('1W').first().fillna(0)
#
# df1 = pd.concat([df1, d1[~d1.index.isin(df1.index)]]).sort_index()
# df2 = pd.concat([df2, d2[~d2.index.isin(df2.index)]]).sort_index()
#
# df1 = df1.reset_index()
# df2 = df2.reset_index()

print('two', len(df1), len(df2))
...
#
Plotname = 'EtaBkg_vs_Eta_alltimerange_unzoom'
...
# Plotname = 'DQA_vs_Raw_alltimerange_v2'
...
Plotname = 'DQA_vs_WOUDC_alltimerange_v2'
...
t = df1.pivot_table(index='PreLevel', columns='date', values='RDif_UcIntLin', fill_value = 0, dropna = True)
