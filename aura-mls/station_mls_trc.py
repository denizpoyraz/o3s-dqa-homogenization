...
ozone = 'O3_trc'
name_out = f'MLS_{scname}Interpolated_nors80_v05_dqa_trc'

# ozone = 'O3_trcc'
# name_out = f'MLS_{scname}Interpolated_nors80_v05_dqa_trcc'
...
for date in date_list:
    ...
    try:
        ...
        df = pd.read_csv(path + "DQA_trc/" + str(date) + "all_trcc_hom.csv")

    except FileNotFoundError:
        try:
            df = pd.read_csv(path + "DQA_trc/" + str(date) + "_all_trcc_hom.csv")
        except FileNotFoundError:
    ...
    # df['Height'] = df['Alt']    df = df[df.O3_trc < 99]
