...
# Import the napB package
import napB

# Let's open a NASA Ames file and examine its contents
f = napB.openNAFile("data_files/2010.na")
...
napB.convertNAToNC(na_file, nc_file, global_attributes=glob_atts)

# Let's convert a NASA Ames file to a CSV and add an annotation column to explain the header
napB.convertNAToCSV(na_file, annotation=True)
...
napB.convertNCToNA("data_files/test1.nc", "test_outputs/test1nc.na", 
...
napB.convertNCToCSV("data_files/test1.nc", "test_outputs/test1nc_no_header.csv",
                     no_header=True)
...
# We also want to instruct napB to overwrite the content of its 
# MNAME (Mission Name) header line with our specific mission name.
# Also, tell napB to write the output to NASA Ames file format index (FFI) 2310
# because we know it is compatible.
napB.convertCDMSObjectsToNA([cdms_var_1, cdms_var_2], [("Institute", "British Atmospheric Data Centre")], 
...
# them to a CSV file.
napB.convertCDMSObjectsToCSV(cdms_vars, global_atttributes, csv_file)

# Let's take a NASA Ames dictionary object, and write it to a NetCDF file
napB.writeNADictToNC(na_dict, nc_file, mode="w")

# Let's try and write a second na_dict object to the same NetCDF file using mode="a".
napB.writeNADictToNC(na_dict_2, nc_file, mode="a")
...
(cdms_vars_primary, cdms_vars_aux, global_attributes) = napB.readCDMSObjectsFromNA(na_file)
...
from .utils.common_utils import getVersion,getDefault
#import napB.utils.compare_na
#from .utils.text_parser

# Bring some utils into the API
#compareNA = napB.utils.compare_na.compareNA
from .utils.common_utils import readFFI,chooseFFI,getNAFileClass,getFileNameWithNewExtension

__version__ = getVersion()
default_delimiter = getDefault("default_delimiter")
default_float_format = getDefault("default_float_format")
...
#def convertNAToNC(na_file, nc_file=None, mode="w", variables=None, aux_variables=None,
    ...
    #variables - is a list of variable names that you wish to be converted. If not set then 
              #napB will attempt to convert all files.
    #aux_var_list - is a list of auxiliary variables names that you wish to be converted. 
              #If not set then napB will use any compatible variables it finds as 
              #auxiliary variables.
    #global_attributes - is a list of global attributes to add to the output file.
    #rename_variables - is a dictionary of {old_name: new_name} variable ID pairs that napB 
    ...
    #import napB.nc_interface.na_to_nc
    #convertor = apply(napB.nc_interface.na_to_nc.NAToNC, [], arg_dict)
    #convertor.convert()
    #if nc_file == None:
...
#def convertNCToNA(nc_file, na_file=None, var_ids=None, na_items_to_override={},
    ...
    #import napB.nc_interface.nc_to_na
    #convertor = apply(napB.nc_interface.nc_to_na.NCToNA, [], arg_dict)
    #convertor.convert()
...
#def convertCDMSObjectsToNA(cdms_vars, global_attributes, na_file, 
    ...
    #no_header - if set to True then only the data blocks are written to file.
    #"""
    #import napB.nc_interface.cdms_objs_to_na_file
    #convertor = napB.nc_interface.cdms_objs_to_na_file.CDMSObjectsToNAFile(cdms_vars, global_attributes=global_attributes, 
...
#def writeNADictToNC(na_dict, nc_file, mode="w"):
    ...
    #na_file_obj = napB.na_file.na_core.NACore()
    #na_file_obj.setNADict(na_dict)
    ## Fake up some required methods
    #def fakeCaller():pass
    #na_file_obj.readData = fakeCaller
    #import napB.nc_interface.na_to_cdms
    #convertor = napB.nc_interface.na_to_cdms.NAToCDMS(na_file_obj)
...
#def readCDMSObjectsFromNA(na_file):
    ...
    #import napB.nc_interface.na_to_cdms
    #convertor = napB.nc_interface.na_to_cdms.NADictToCdmsObjects(na_file_obj)
...
#def getCDMSVariableFromNA(na_file, var):
    ...
    #na_file_obj = openNAFile(na_file)
    #import napB.nc_interface.na_to_cdms
    #convertor = napB.nc_interface.na_to_cdms.NADictToCdmsObjects(na_file_obj, variables=[var])
    #(cdms_primary_vars, cdms_aux_vars, global_attributes) = convertor.convert()
