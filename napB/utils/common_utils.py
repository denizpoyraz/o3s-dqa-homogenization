...
Functions and classes commonly used in napB.
...
def getNAFileClass(ffi):
    ...
    """
    mod = "napB.na_file.na_file_" + repr(ffi)
    cls = "NAFile" + repr(ffi)
    exec("import %s" % mod)
...
def readFFI(filename):
    ...
    try: ffi = text_parser.readItemsFromLine(topline, 2, int)[-1]
    except: ffi = text_parser.readItemsFromLine(secondline, 2, int)[-1]
    return ffi
