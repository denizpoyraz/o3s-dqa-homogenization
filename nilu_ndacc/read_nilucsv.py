...
def o3tocurrent(dft):
    ...
    # print(dft.at[dft.first_valid_index(), 'iB0'], dft.at[dft.first_valid_index(), 'iB2'], dft.at[dft.first_valid_index(), 'BkgUsed'] )
    ...
    # if (dft.at[dft.first_valid_index(), 'BkgUsed'] != 'Ibg1') & (dft.at[dft.first_valid_index(), 'iB0'] < 9): dft['ibg'] = dft['iB0']
    # if (dft.at[dft.first_valid_index(), 'BkgUsed'] != 'Ibg1') & (dft.at[dft.first_valid_index(), 'iB0'] > 9) \
    #         & (dft.at[dft.first_valid_index(), 'iB2'] < 9): dft['ibg'] = dft['iB2']



    dft.loc[spc,'ibg'] = ComputeIBG(dft[spc], 'ibg')
    dft.loc[ensci,'ibg'] = ComputeIBG(dft[ensci], 'ibg')
...
def ComputeCef(dft):
    ...
    #     VecInterpolate(VecP_ECC6A, VecC_ECC6A_30, dft[(spc) & (sol3)], 0)
    ...
    # print('test in cef function zero',len(dft), dft[dft.Pair == 8.8][['Pair', 'Cef', 'T', 'Tbox']])
    ...
    for k in range(len(dft)):
        ...
        for i in range(len(XValues)-1):
            ...
            if (XValues[i] <= dft.at[k, 'Pair'] <= XValues[i + 1]):
                ...
                # values[k] = y1 + (dft.at[k,'Pair'] - x1) * (y2 - y1) / (x2 - x1)
                ...
                # print(k, dft.at[k,'Pair'],dft.at[k,'Cef'])
