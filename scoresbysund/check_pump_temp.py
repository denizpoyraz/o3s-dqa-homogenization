...
for k in range(len(km_low)):
    ...
    # plt.savefig(path + 'Plots/TPump/NDACC_monthly' + str(km_low[k]) + '.png')
    # plt.show()
    plt.close()
    #
    fig, ax = plt.subplots(figsize=(17, 9))
    ...
    # plt.savefig(path + 'Plots/TPump/DQA_monthly_' + str(km_low[k]) + '.png')
    plt.show()
    plt.close()
...
plot_title = 'Scoresbysund Pump temperature '
...
plt.xlim([280,305])
# plt.plot(avg_km, km_low, color = 'tab:blue', label = 'NDACC before 2016')
# plt.plot( avgcor_km,km_low, color = 'tab:orange', label = 'DQA before 2016')
plt.plot(avg_km, km_low, label = 'NDACC before 2016')
# plt.plot( avgcor_km,km_low,label = 'DQA before 2016')

plt.plot(avg_kmcp, km_low, label = 'NDACC after 2016')
# plt.plot( avgcor_kmcp,km_low,label = 'DQA after 2016')
