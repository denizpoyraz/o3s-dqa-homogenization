...
# file_dfmain = "/home/poyraden/Analysis/Homogenization_public/Files/madrid/DQA_trc/Madrid_AllData_woudc.hdf"
file_dfmain = "/home/poyraden/Analysis/Homogenization_public/Files/madrid/DQA_trc/Madrid_AllData_woudc.csv"
...
#for missing iB0 values for the trc method
dfmetat = dfmeta[dfmeta.iB0 < 0.4]
median_ib0 = dfmetat.iB0.median()
dfmeta.loc[(dfmeta.iB0 > 0.4) | (dfmeta.iB0.isnull()!= 'True'), 'iB0'] = median_ib0
...
for (filename) in (allFiles):
    ...
    if sonde_tbc=='ENSCI05':
        beta = 0.017
        ac = a[0]
        bc = b[0]

    sigma = round(0.2 * tfast_ecc, 1)
    timew = round(3 * sigma, 1)


    ##trc applied to current
    try:
        df['IminiB0'] = df['I'] - df['iB0']
    except KeyError:
        df['iB0'] = dfm.at[0,'iB0']
        df['IminiB0'] = df['I'] - df['iB0']

    Imib0_s_ghv = smooth_gaussian(np.array(df['Time'].tolist()), np.array(df['IminiB0'].tolist()), timew, sigma)
    I_s_ghv = smooth_gaussian(np.array(df['Time'].tolist()), np.array(df['I'].tolist()), timew, sigma)

    df['IminiB0_gsm'] = Imib0_s_ghv
    df['I_gsm'] = I_s_ghv
    df = df.reset_index()
    
    Islow_gs, Islow_conv_gs,Ifastminib0_gs, Ifastminib0_deconv_gs = \
        convolution(df, 'IminiB0_gsm', 'I_gsm', 'Time', beta, 1, sonde_tbc)
    df['Ifast_minib0_deconv_gs'] = Ifastminib0_deconv_gs
    
    df['R'] = ac + bc * np.log10(df['Pair'])
    df['I_corrected'] = df['Ifast_minib0_deconv_gs'] * (100 / (100 + df['R']))

    # all corrections
    df['O3c'] = currenttopo3(df, 'I', 'Tpump_cor', 'iBc', 'eta_c', 'Phip_cor')
    df['O3_trc'] = currenttopo3(df, 'Ifast_minib0_deconv_gs', 'Tpump_cor', 'iBc', 'eta_c', 'Phip_cor')
    df['O3_trcc'] = currenttopo3(df, 'I_corrected', 'Tpump_cor', 'iBc', 'eta_c', 'Phip_cor')
    ...
    if dfa['Pair'].min() < 33:
        ...
        # the same for trcc o3 values
        dfm['O3Sonde_trcc'] = o3_integrate(dfa, 'O3_trcc')
        dfm['O3SondeTotal_trcc'] = dfm['O3Sonde_trcc'] + dfm['ROC']
        try:
            dfm['O3ratio'] = dfm['TotalO3_Col2A'] / dfm['O3SondeTotal']
            dfm['O3ratio_hom'] = dfm['TotalO3_Col2A'] / dfm['O3SondeTotal_hom']
            dfm['O3ratio_trcc'] = dfm['TotalO3_Col2A'] / dfm['O3SondeTotal_trcc']

            if dfm.at[0, 'TotalO3_Col2A'] > 999:
                dfm['O3ratio'] = 9999
                dfm['O3ratio_hom'] = 9999
                dfm['O3ratio_raw'] = 9999
                dfm['O3ratio_trcc'] = 9999
    ...
    df.to_csv(path + filefolder + date_tmp + "_all_trcc_hom.csv")
