...
path = '/home/poyraden/Analysis/Homogenization_public/Files/valentia/WOUDC_TO/'

name_out = 'to_valentia_alldata'
name_mout = 'to_valentia_metadata'
...
for filename in allFiles:
    print(filename)
    tmp = filename.split('.')[0][-6:]
    print('tmp', tmp)
    if tmp == 'MD010613': continue
    if tmp == '201908': continue
    if tmp == '202011': continue
    ...
    try:
        ...
        profile_keys = extcsv_to.sections['DAILY'].keys()
        Profile = extcsv_to.sections['DAILY']['_raw']
        del extcsv_to.sections['DAILY']
...
#
dfall.to_csv(path + "/TO/" + name_out + ".csv")
dfall.to_hdf(path + "/TO/" + name_out + ".h5", key='df')
dfmall.to_hdf(path + "/TO/" + name_mout + ".h5", key='df')
dfmall.to_csv(path + "/TO/" + name_mout + ".csv")
